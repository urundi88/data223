...
class VideoEditorApp:
    def __init__(self, root):
        ...
        self.point_a = None  # New: To store Point A
        self.point_b = None  # New: To store Point B
        self.initial_canvas_width = 800  # Fixed initial width
        self.initial_canvas_height = 600  # Fixed initial height
        self.load_saves()
        self.load_goals()
        ...
        # Canvas para exibir o vídeo com tamanho fixo
        self.canvas_frame = tk.Frame(self.right_frame, bg='#2a2a2a', height=self.initial_canvas_height)
        self.canvas_frame.pack(side=tk.TOP, fill=tk.X)
        self.canvas = tk.Canvas(self.canvas_frame, bg='#2a2a2a', width=self.initial_canvas_width, height=self.initial_canvas_height, highlightthickness=0)
        self.canvas.pack()
        ...
        # New buttons for Point A and Point B
        self.take_point_a_button = tk.Button(self.button_frame, text="Take Point A", command=self.set_point_a, bg='#4a4a4a', fg="#ffffff", state=tk.DISABLED)
        self.take_point_a_button.pack(side=tk.LEFT, padx=5)

        self.take_point_b_button = tk.Button(self.button_frame, text="Take Point B", command=self.set_point_b, bg='#4a4a4a', fg="#ffffff", state=tk.DISABLED)
        self.take_point_b_button.pack(side=tk.LEFT, padx=5)

        self.play_selected_clip_button = tk.Button(self.button_frame, text="Play Selected Clip", command=self.play_selected_clip, bg='#4a4a4a', fg="#ffffff")
        self.play_selected_clip_button.pack(side=tk.LEFT, padx=5)
    ...
    def open_video(self):
        ...
        if file_path:
            ...
            self.take_point_a_button.config(state=tk.NORMAL)  # Enable Point A button when video is loaded
            try:
    ...
    def update_frame(self):
        if self.cap and self.cap.isOpened() and (self.last_frame_time < 0 or time.time() - self.last_frame_time >= 0.1):
            try:
                ...
                # Apply zoom but keep within canvas bounds
                h, w = frame.shape[:2]
                new_h, new_w = int(h * self.zoom_level), int(w * self.zoom_level)
                frame = cv2.resize(frame, (min(new_w, self.initial_canvas_width), min(new_h, self.initial_canvas_height)), 
                                 interpolation=cv2.INTER_LINEAR if self.zoom_level < 5 else cv2.INTER_NEAREST)
                
                # Convert to RGB and create PhotoImage
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                img = Image.fromarray(frame_rgb)
                self.photo = ImageTk.PhotoImage(image=img)
                # Use canvas dimensions instead of resizing canvas
                self.canvas.create_image(self.initial_canvas_width // 2, self.initial_canvas_height // 2, image=self.photo)
    ...
    def set_point_a(self):
        if not self.is_video_loaded:
            ...
            return
        self.point_a = self.current_frame
        messagebox.showinfo("Info", f"Point A set at frame {self.point_a}")
        self.take_point_a_button.config(state=tk.DISABLED)
        self.take_point_b_button.config(state=tk.NORMAL)

    def set_point_b(self):
        if not self.is_video_loaded or self.point_a is None:
            messagebox.showerror("Error", "Set Point A first.")
            return
        self.point_b = self.current_frame
        if self.point_b <= self.point_a:
            messagebox.showerror("Error", "Point B must be after Point A.")
            return
        clip_name = f"clip_{self.clip_count}"
        self.temp_clips[clip_name] = {"start": self.point_a, "end": self.point_b, "loop": 0}
        self.save_clip_to_file(clip_name, self.point_a, self.point_b)
        self.update_clip_list()
        self.update_clip_controls()
        self.clip_count += 1
        self.point_a = None
        self.point_b = None
        self.take_point_b_button.config(state=tk.DISABLED)
        self.take_point_a_button.config(state=tk.NORMAL)
        messagebox.showinfo("Success", f"Clip {clip_name} saved from frame {self.point_a} to {self.point_b}")

    def play_selected_clip(self):
        if not self.is_video_loaded or self.is_playing:
            messagebox.showerror("Error", "No video loaded or already playing.")
            return
        selected = self.clip_listbox.get(self.clip_listbox.curselection())
        if not selected:
            messagebox.showerror("Error", "Select a clip to play.")
            return
        try:
            self.custom_fps = float(self.fps_entry.get())
            ...
            messagebox.showerror("Error", "Enter a valid FPS.")
            return
        self.current_clip = selected
        self.is_playing = True
        self.play_thread = threading.Thread(target=self._play_clip_thread, args=(selected,))
        self.play_thread.daemon = True
        self.play_thread.start()
